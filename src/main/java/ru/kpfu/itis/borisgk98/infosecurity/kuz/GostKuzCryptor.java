...
public class GostKuzCryptor implements BlockCryptor {
    ...
    static public byte[][] F(byte[] in_key_1, byte[] in_key_2, byte[] iter_const)
    {
        ...
        internal = S(internal);
        internal = L(internal);
        byte[] out_key_1 = X(internal, in_key_2);
        byte key[][] = new byte[2][];
        key[0] = out_key_1;
        key[1] = out_key_2;
        return key;
    ...
    {
        if (block.length % getBlockSize() != 0) {
    ...
    public byte[] decrypt(byte[] block)
    {
        ...
        for(i = 8; i >= 0; i--)
        {
            out_blk = reverseL(out_blk);
            out_blk = reverseS(out_blk);
            out_blk = X(iter_key[i], out_blk);
        }
        return out_blk;
    }
